...
class PegSpec:
    ...
    # Length meant to be inserted into the hole
    insertion_depth: float = 0.016

    # Frames of the tip
    tip_pos = (0.0, 0.0, total_length)
    tip_rot = quat_from_euler_xyz(
        torch.tensor(torch.pi), torch.tensor(0.0), torch.tensor(0.0)
    ).tolist()
