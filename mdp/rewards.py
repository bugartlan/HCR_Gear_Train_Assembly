...
def keypoint_dist_reward(
    env: ManagerBasedRLEnv,
    length: float,
    n_points: int = 4,
    std: float = 0.1,
    asset1_cfg: SceneEntityCfg = SceneEntityCfg("held_asset_frame"),
    asset2_cfg: SceneEntityCfg = SceneEntityCfg("target_frame"),
) -> torch.Tensor:
    """Reward based on the distance between keypoints on two assets."""
    ...
    dist = distance(env.num_envs, n_points, length, asset1, asset2, env.device)
    return torch.exp(-dist.pow(2) / std**2).mean(dim=0)  # (num_envs,)
...
def success_bonus(
    ...
    """Give a bonus reward for successful alignment of two assets."""
    asset1: Articulation | FrameTransformer = env.scene[asset1_cfg.name]
    asset2: Articulation | FrameTransformer = env.scene[asset2_cfg.name]

    dist = distance(env.num_envs, n_points, length, asset1, asset2, env.device)
    return (dist.mean(dim=0) < threshold).float()
