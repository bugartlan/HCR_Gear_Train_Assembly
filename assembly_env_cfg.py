...
class AssemblySceneCfg(InteractiveSceneCfg):
    ...
    held_asset_frame: FrameTransformerCfg = MISSING
...
class ActionsCfg:
    ...
    # TODO: conform action space to real robot limits
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        fixed_asset_pos = ObsTerm(
            func=mdp.fixed_asset_pos_wrt_robot,
            noise=GaussianNoiseCfg(mean=0.0, std=0.001),
        )
    ...
    #     held_asset_pos = ObsTerm(func=mdp.peg_pos_wrt_robot)
    #     held_asset_vel = ObsTerm(func=mdp.peg_vel_wrt_robot)
...
class EventCfg:
    ...
    reset_fixed_asset = EventTerm(
        ...
        params={
            ...
            "velocity_range": {},
            "asset_cfg": SceneEntityCfg("fixed_asset", body_names="Hole"),
        },
    )
...
class RewardsCfg:
    keypoint_distance_baseline = RewTerm(
        func=mdp.keypoint_dist_reward, weight=1.0, params={"std": 0.1}
    )

    keypoint_distance_coarse = RewTerm(
        func=mdp.keypoint_dist_reward, weight=1.0, params={"std": 0.04}
    )

    keypoint_distance_fine = RewTerm(
        func=mdp.keypoint_dist_reward, weight=1.0, params={"std": 0.01}
    )
    ...
    slip = RewTerm(
        ...
        params={
            "n_points": 2,
            "std": 0.01,
            "asset1_cfg": SceneEntityCfg("ee_frame"),
            "asset2_cfg": SceneEntityCfg("held_asset"),
        },
    )
...
class TerminationsCfg:
    ...
    dropped = DoneTerm(func=mdp.dropped, params={"threshold": 0.05})
